#include <atlas/bullet/clock.h>

template <class T>
const T &ClockMin(const T &a, const T &b)
{
    return a < b ? a : b;
}
...
void Clock::reset(bool zeroReference)
{
    if (zeroReference)
    {
#ifdef B3_USE_WINDOWS_TIMERS
...
double Clock::getTimeInSeconds()
{
    return double(getTimeMicroseconds() / 1.e6);
}

void Clock::usleep(int microSeconds)
{
#ifdef _WIN32
    if (microSeconds == 0)
    {
